  ...
  return (
    <div className="bg-background min-h-screen text-white pb-24">
      ...
      <div className="fixed top-0 left-0 w-full p-6 z-50 flex justify-between items-center mix-blend-difference text-white pointer-events-none">
         <Link to="/" className="flex items-center gap-2 pointer-events-auto group clickable">
            ...
            <span className="font-mono text-sm hidden md:inline opacity-0 group-hover:opacity-100 transition-opacity -translate-x-2 group-hover:translate-x-0">BACK TO INDEX</span>
         </Link>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-24">
            ...
            <div className="lg:col-span-4">
               <div className="sticky top-24 space-y-12">
                  ...
                  <div>
                     <h3 className="font-display text-xl font-bold mb-4">Software Used</h3>
                     <div className="flex flex-wrap gap-2">
                        {project.software.map(sw => (
                           <span key={sw} className="px-3 py-1 border border-white/20 rounded-full text-xs font-mono uppercase hover:bg-white hover:text-black transition-colors cursor-default">
                              {sw}
                           </span>
                        ))}
                     </div>
            ...
            <div className="lg:col-span-8 space-y-24">
               ...
               <div className="space-y-4">
                  {project.images.beauty.map((img, idx) => (
                     <div key={idx} className="relative group overflow-hidden">
                        <img src={img} alt={`Render ${idx}`} className="w-full h-auto transition-transform duration-700 group-hover:scale-[1.02]" />
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#111] p-8 flex flex-col justify-between min-h-[400px]">
                     <div className="font-display text-3xl font-bold text-gray-700">Topology<br/>Breakdown</div>
                     <div className="font-mono text-xs text-gray-500">
                        Optimized edge flow for animation deformation and game engine performance.
                     </div>
                  </div>
                  <div className="aspect-square bg-black relative group overflow-hidden">
                     <img src={project.images.wireframe} alt="Wireframe" className="w-full h-full object-cover invert opacity-80 group-hover:opacity-100 transition-opacity" />
                     <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-6">
                        <span className="font-mono text-xs text-green-400">WIREFRAME_VIEW</span>
                     </div>
                  </div>
               </div>
                     ...
                     {project.images.textures.map((tex) => (
                        <div key={tex.name} className="space-y-2">
                           <div className="aspect-square bg-[#1a1a1a] overflow-hidden group">
                              <img src={tex.url} alt={tex.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
                           </div>
                           <p className="font-mono text-[10px] uppercase text-center text-gray-500">{tex.name}</p>
                        </div>
                     ))}
                  </div>
               </div>
      ...
      <div className="max-w-[1800px] mx-auto px-6 md:px-12 mt-32 border-t border-white/20 pt-12 text-center">
         <p className="font-mono text-xs text-gray-500 mb-4">THANK YOU FOR WATCHING</p>
         <Link to="/" className="inline-block font-display text-4xl md:text-6xl font-black uppercase hover:text-gray-400 transition-colors clickable">
            Next Project <ArrowDown className="inline ml-4 rotate-[-90deg]" size={48}/>
         </Link>
      </div>
